...
planeID = p.loadURDF(osp.join(pybullet_data.getDataPath(), "plane.urdf"))
useRealTimeSim = 0
...
car = p.loadURDF(osp.join(pybullet_data.getDataPath(), "racecar/racecar_differential.urdf"))
...
obstacles = [p.loadURDF(folder.format(i)) for i in items]
...
# boxId = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=geomBox, basePosition=np.r_[2, 0, 0.2])
